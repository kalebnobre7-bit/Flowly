...
window.toggleTaskExpansion = function (task, el) {
    const isExpanded = el.nextElementSibling && el.nextElementSibling.classList.contains('task-expansion');
    ...
    const dateStr = el.dataset.date;   // data-date, not data-date-str
    ...
    exp.style.transition = 'all 180ms ease-out';
    exp.style.marginLeft = '28px'; // Align with task text
    exp.style.padding = '0px 12px';
    exp.style.borderRadius = '6px';
    exp.style.fontSize = '12px';
    exp.style.color = 'var(--text-tertiary)';
    ...
    const createRow = (labelTxt) => {
        ...
        lbl.style.minWidth = '54px';
        row.appendChild(lbl);
        return row;
    };

    // 1. TYPE PILLS
    const typeRow = createRow('Tipo');
    ...
    // 2. PRIORITY PILLS
    const prioRow = createRow('Pr/dade');
    ...
    const priosContainer = document.createElement('div');
    priosContainer.style.display = 'flex';
    priosContainer.style.gap = '4px';
    ...
    prios.forEach(p => {
        ...
        priosContainer.appendChild(btn);
    });
    prioRow.appendChild(priosContainer);
    exp.appendChild(prioRow);

    // 3. REPEAT DAYS
    const repRow = createRow('Repetir');
    ...
    repContainer.style.gap = '4px';

    days.forEach((d, i) => {
        const btn = document.createElement('button');
        btn.className = 'inline-pill inline-day-pill';
        btn.textContent = d;
    ...
    repRow.appendChild(repContainer);
    ...
    delBtn.className = 'inline-pill';
    delBtn.innerHTML = '<i data-lucide="trash-2" style="width:12px;height:12px;margin-right:4px;"></i> Excluir';
    ...
    requestAnimationFrame(() => {
        exp.style.opacity = '1';
        exp.style.maxHeight = '250px';
        exp.style.marginTop = '2px';
...
window.deleteTaskInline = async function (task, dateStr, period, _indexStr, isRecurring) {
    if (confirm('Excluir esta tarefa definitivamente?')) {
        ...
        if (window._rtTimeout) { clearTimeout(window._rtTimeout); window._rtTimeout = null; }
        let deleted = false;
        ...
        if (!isRecurring) {
            ...
            if (allTasksData[dateStr]?.[period]) {
                const localIdx = allTasksData[dateStr][period].findIndex(t =>
                    (task.supabaseId && t.supabaseId === task.supabaseId) || t.text === task.text
                );
                if (localIdx >= 0) {
...
function saveToLocalStorage() {
    ...
    // Sincronização em massa não deve ocorrer a cada saveToLocalStorage,
    // as funções de edição específicas devem chamar sync explicitamente.
}
...
async function loadDataFromSupabase() {
    ...
    // Load custom types, priorities and settings safely (might not exist yet)
    try {
        ...
        if (typesRes.data) customTaskTypes = typesRes.data;
        if (priosRes.data) customTaskPriorities = priosRes.data;
        if (settingsRes.data) {
            dbUserSettings = {
    ...
    if (error) {
        ...
        alert('Erro crítico ao carregar tarefas do servidor:\n' + (error.message || JSON.stringify(error)));
        return; // Evita limpar a memória local caso a query simplesmente falhe
    }

    // Snapshot local BEFORE wiping, to recover completed states that may not have synced yet
    const localSnapshot = (() => {
        try { return JSON.parse(localStorage.getItem('allTasksData') || 'null'); } catch (e) { return null; }
    })();
        ...
        tasks.forEach(task => {
            ...
            if (task.day === 'RECURRING') {
                remoteRecurringTasks.push({
                    ...
                    priority: null, // Prioridade padrao se nao tiver
                    color: task.color || 'default',
                    type: task.type || null,
    ...
    // Merge: recover completed=true states that may not have synced yet before reload
    if (localSnapshot) {
        Object.entries(localSnapshot).forEach(([dateStr, periods]) => {
            if (!periods || typeof periods !== 'object') return;
            Object.entries(periods).forEach(([period, tasks]) => {
                if (!Array.isArray(tasks)) return;
                tasks.forEach(localTask => {
                    if (!localTask.completed) return;
                    const serverTasks = allTasksData[dateStr]?.[period];
                    if (!serverTasks) return;
                    const serverTask = serverTasks.find(t =>
...
async function syncTaskToSupabase(dateStr, period, task) {
    ...
    try {
        ...
        if (task.supabaseId && task.supabaseId.includes('-')) {
            // UPDATE: only send mutable fields — avoids RLS issues with user_id/day/period
            const updatePayload = {
                text: task.text,
                completed: task.completed === true,
                color: task.color || 'default',
            ...
            ({ data, error } = await supabaseClient.from('tasks').update(updatePayload).eq('id', task.supabaseId).select());
        } else {
            // INSERT: include all required fields
            const insertPayload = {
                user_id: currentUser.id,
                day: dateStr,
                period: period,
...
async function deleteTaskFromSupabase(task, day, period) {
    ...
    try {
        // Delete by supabaseId (exact row) when available
        if (task.supabaseId) {
        ...
        // Also delete by text+day+period to catch any duplicates without IDs
...
function renderAnalyticsView() {
    ...
    weekDates.forEach(({ name, dateStr }) => {
        ...
        Object.entries(dayTasks).forEach(([period, tasks]) => {
            ...
            if (Array.isArray(tasks)) {
                tasks.forEach(task => {
                    dayTotal++; totalTasksWeek++;
                    if (task.completed) { dayCompleted++; completedTasksWeek++; }
                    if (dateStr === today) { totalTasksToday++; if (task.completed) completedTasksToday++; }
                });
            }
        });
        const routineForDay = getRoutineTasksForDate(dateStr);
        dayTotal += routineForDay.length; totalTasksWeek += routineForDay.length;
    ...
    for (let i = 0; i < 60; i++) {
        ...
        Object.entries(cTasks).forEach(([p, tasks]) => {
            if (p === 'Rotina') return;
            if (Array.isArray(tasks)) { st += tasks.length; sc += tasks.filter(t => t.completed).length; }
        });
        const rfc = getRoutineTasksForDate(cStr);
        st += rfc.length; sc += rfc.filter(t => t.completed).length;
        if (st > 0 && sc === st) { currentStreak++; }
        else if (i > 0 || (i === 0 && st > 0)) { break; }
    ...
    lastWeekDates.forEach(({ dateStr }) => {
        ...
        Object.entries(dt).forEach(([p, tasks]) => {
            if (p === 'Rotina') return;
            if (Array.isArray(tasks)) { lastWeekTotal += tasks.length; lastWeekCompleted += tasks.filter(t => t.completed).length; }
        });
        const rlw = getRoutineTasksForDate(dateStr);
        lastWeekTotal += rlw.length; lastWeekCompleted += rlw.filter(t => t.completed).length;
    });
    ...
    const dayPerfData = weekDates.map(({ name, dateStr }, idx) => {
        ...
        const color = isToday ? '#0A84FF' : rate >= 80 ? '#30D158' : rate >= 50 ? '#0A84FF' : rate > 0 ? '#FF9F0A' : 'rgba(255,255,255,0.08)';
    ...
    const worstDay = dayRates.length > 1 ? dayRates.reduce((w, d) => d.rate < w.rate ? d : w) : null;
    ...
    // ── Habit ranking ─────────────────────────────────────────────────────
    const habitRanking = allHabitsArr.map(h => ({
        text: h.text,
        rate: getHabitCompletionRate(h.text, 30),
        streak: getHabitStreak(h.text)
    })).sort((a, b) => b.rate - a.rate);

    const habitRankingHTML = habitRanking.length === 0
        ...
        : habitRanking.slice(0, 8).map((h, i) => {
            const medals = ['🥇','🥈','🥉'];
    ...
    const catHTML = Object.keys(catCounts).length === 0
        ? `<div style="padding:16px 0;text-align:center;color:var(--text-tertiary);font-size:13px">Sem dados de categoria esta semana.</div>`
        : Object.entries(catCounts).sort((a,b) => b[1].total - a[1].total).map(([id, c]) => {
            const ti = taskTypes.find(t => t.id === id) || { name: id === 'OTHER' ? 'Sem categoria' : id, color: '#86868b' };
    ...
    // ── 30-day heatmap ─────────────────────────────────────────────────────
    let heatmapCells = '';
    const heatCur = new Date(); heatCur.setDate(heatCur.getDate() - 29);
    for (let i = 0; i < 30; i++) {
        ...
        Object.entries(dt).forEach(([p, tasks]) => {
            if (p === 'Rotina') return;
            if (Array.isArray(tasks)) { ht += tasks.length; hc += tasks.filter(t => t.completed).length; }
        });
        const rh = getRoutineTasksForDate(dStr); ht += rh.length; hc += rh.filter(t => t.completed).length;
        const rate = ht > 0 ? Math.round((hc / ht) * 100) : 0;
        const bg = rate >= 80 ? '#30D158' : rate >= 60 ? '#0A84FF' : rate >= 40 ? '#FF9F0A' : rate > 0 ? 'rgba(255,69,58,0.55)' : 'rgba(255,255,255,0.06)';
    ...
    const dayPerfHTML = dayPerfData.map(d => {
    ...
    if (totalTasksToday > 0 && completedTasksToday === totalTasksToday)
        insights.push({ color:'green', icon:'✅', title:'Dia Perfeito!', text:'Todas as tarefas de hoje concluídas. Excelente!' });
    if (currentStreak >= 3)
        insights.push({ color:'orange', icon:'🔥', title:`${currentStreak} Dias de Streak`, text:'Dias consecutivos com 100% das tarefas concluídas!' });
    if (habitRate === 100 && totalHabits > 0)
        insights.push({ color:'purple', icon:'✨', title:'Hábitos Perfeitos', text:'Todos os hábitos marcados hoje. Consistência máxima!' });
    if (weekDiff >= 10)
        insights.push({ color:'green', icon:'📈', title:'Semana em Alta', text:`+${weekDiff}% vs semana anterior. Crescendo consistentemente!` });
    if (weekDiff <= -10)
        insights.push({ color:'red', icon:'📉', title:'Queda de Desempenho', text:`${weekDiff}% vs semana anterior. Identifique o que está bloqueando.` });
    if (bestDay && bestDay.rate >= 80)
        insights.push({ color:'blue', icon:'⭐', title:`Destaque: ${bestDay.name}`, text:`${bestDay.rate}% de conclusão — seu melhor dia da semana!` });
    if (insights.length === 0 && weekRate > 0)
        insights.push({ color:'blue', icon:'💡', title:'Continue Evoluindo', text:`${weekRate}% de conclusão esta semana. Cada dia conta!` });
    if (insights.length === 0)
        insights.push({ color:'blue', icon:'🚀', title:'Comece Hoje', text:'Adicione tarefas e hábitos para ver seus insights aqui.' });
    ...
    const nowYear = nowDate.getFullYear(), nowMonth = nowDate.getMonth();
    const daysInMonth = new Date(nowYear, nowMonth + 1, 0).getDate();
    const todayDay = nowDate.getDate();
    const monthLabels = [], monthData = [], monthPointColors = [];
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        Object.entries(dt).forEach(([p, tasks]) => {
            if (p === 'Rotina') return;
            if (Array.isArray(tasks)) { mt += tasks.length; mc += tasks.filter(t => t.completed).length; }
        });
        const rm = getRoutineTasksForDate(dStr); mt += rm.length; mc += rm.filter(t => t.completed).length;
    ...
    const monthAvgRate = monthAvg.length > 0 ? Math.round(monthAvg.reduce((a, b) => a + b, 0) / monthAvg.length) : 0;
    ...
    view.innerHTML = `<div class="analytics-container-v2">
        ...
        <div class="analytics-kpi-grid-v2">
            ...
            <div class="analytics-kpi-v2">
                ...
                <span class="analytics-kpi-v2-badge ${trendClass}">${trendLabel} vs sem. ant.</span>
            </div>
            <div class="analytics-kpi-v2">
    ...
    setTimeout(() => {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            ...
                            titleColor: '#ccc', bodyColor: '#fff', padding: 10,
                            callbacks: { label: item => ` ${item.raw !== null ? item.raw + '%' : 'Sem dados'}` }
                        }
                    },
                    scales: {
                        y: { beginAtZero: true, max: 100, grid: { color: chartDefaults.gridColor }, ticks: { color: chartDefaults.color, callback: v => v + '%' }, border: { display: false } },
                        x: { grid: { display: false }, ticks: { color: chartDefaults.color }, border: { display: false } }
                    }
                }
            });
        }
        ...
        if (hCtx) {
            const doughnutData = totalHabits > 0
                ? [completedHabitsToday, Math.max(0, totalHabits - completedHabitsToday)]
                : [0, 1];
            const doughnutColors = totalHabits > 0
        ...
        if (monthCtx) {
            new Chart(monthCtx, {
                ...
                options: {
                    ...
                    scales: {
                        y: { beginAtZero: true, max: 100, grid: { color: chartDefaults.gridColor }, ticks: { color: chartDefaults.color, callback: v => v + '%', stepSize: 25 }, border: { display: false } },
                        x: {
                            grid: { display: false },
                            ticks: { color: chartDefaults.color, maxTicksLimit: 12,
                                callback: function(val, index) { const d = index + 1; return d === 1 || d % 5 === 0 || d === daysInMonth ? d : ''; }
                            },
...
function renderRoutineView(embeddedEl) {
    ...
    const ringColor         = todayPercent >= 80 ? '#30D158' : todayPercent >= 50 ? '#0A84FF' : todayPercent > 0 ? '#FF9F0A' : 'rgba(255,255,255,0.12)';

    // Weekly stats (last 7 days)
    let totalWeekSch = 0, totalWeekComp = 0;
    const weeklyDayData = [];
    for (let i = 6; i >= 0; i--) {
        ...
        if (cnt > 0) { totalWeekSch += cnt; totalWeekComp += comp; }
        weeklyDayData.push({ abbr: DAY_ABBR[d.getDay()], init: DAY_INIT[d.getDay()], rate: cnt > 0 ? Math.round((comp/cnt)*100) : 0, total: cnt, completed: comp, isToday: i === 0, dow: d.getDay() });
    }
    ...
    for (let i = 0; i < 365; i++) {
        const d = new Date(today); d.setDate(today.getDate() - i);
        const tasks = getRoutineTasksForDate(localDateStr(d));
        if (tasks.length > 0 && tasks.filter(t => t.completed).length === tasks.length) { currentStreak++; }
        else if (i > 0) break;
    }
    ...
    for (let i = 0; i < 30; i++) {
        const d = new Date(today); d.setDate(today.getDate() - i);
        const tasks = getRoutineTasksForDate(localDateStr(d));
        if (tasks.length > 0 && tasks.filter(t => t.completed).length === tasks.length) perfDayCount[d.getDay()]++;
    }
    ...
    for (let w = 11; w >= 0; w--) {
        ...
        for (let d = 6; d >= 0; d--) {
            ...
            if (tasks.length > 0) {
                ...
                bg = rate >= 1 ? '#30D158' : rate >= 0.7 ? 'rgba(48,209,88,0.55)' : rate >= 0.4 ? '#FF9F0A' : 'rgba(255,69,58,0.45)';
            }
    ...
    const heatHTML = heatWeeks.map((week, wi) =>
        `<div class="routine-heatmap-week-row">${week.map(cell =>
    ...
    // ── Habits HTML ────────────────────────────────────────────────────────────
    const habitsHTML = activeRoutines.length === 0
        ...
        : activeRoutines.map(task => {
            let itemTotal = 0, itemCompleted = 0, runningStreak = 0, streakBroken = false;
            for (let i = 0; i < 30; i++) {
                const d = new Date(today); d.setDate(today.getDate() - i);
                const dStr = localDateStr(d);
                if (task.daysOfWeek.includes(d.getDay())) {
                    ...
                    if (done) { itemCompleted++; if (!streakBroken) runningStreak++; }
            ...
            const rateColor = itemRate >= 80 ? '#30D158' : itemRate >= 50 ? '#0A84FF' : 'var(--text-tertiary)';
            const typeInfo = taskTypes.find(t => t.id === task.type);
            const tagHTML = typeInfo ? `<span class="routine-habit-tag" style="background:${typeInfo.color}18;color:${typeInfo.color};border-color:${typeInfo.color}30">${typeInfo.name}</span>` : '';
            const priorityInfo = getTaskPriorities().find(p => p.id === task.priority);
            const prioHTML = priorityInfo ? `<span class="routine-habit-tag" style="background:${priorityInfo.color}15;color:${priorityInfo.color};border-color:${priorityInfo.color}28">${priorityInfo.name}</span>` : '';
            const dayDots = [0,1,2,3,4,5,6].map(dow => {
    ...
    const periodHTML = periods.map(p => {
        ...
        const tot = tasks.length, done = tasks.filter(t => t.completed).length;
    ...
    // ── Weekly column view ────────────────────────────────────────────────────
    const weeklyColsHTML = weeklyDayData.map(day => {
        const barH = day.rate > 0 ? Math.max(6, Math.round(day.rate * 0.8)) : 0;
        const barColor = day.isToday ? '#0A84FF' : day.rate >= 80 ? '#30D158' : day.rate >= 50 ? '#0A84FF' : day.rate > 0 ? '#FF9F0A' : 'rgba(255,255,255,0.08)';
    ...
    // ── Streak Badge ──────────────────────────────────────────────────────────
    const streakBadge = currentStreak >= 7
...
async function renderInlineEditors() {
    ...
    const renderItem = (item, type, container, arr, dbTable) => {
        ...
        inputName.className = 'bg-black/30 border border-white/10 rounded px-2 py-1 text-sm text-white focus:border-blue-500 outline-none flex-1 transition-colors';
        ...
        const triggerUpdate = () => {
            ...
            pendingUpdate = setTimeout(async () => {
                ...
                if (currentUser) {
                    ...
                    await supabaseClient.from(dbTable).upsert({ id: item.id, name: item.name, color: item.color, user_id: currentUser.id });
                }
            }, 800);
        };
        ...
        btnDelete.className = 'text-gray-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity p-1';
    ...
    priosList.innerHTML = '';
    customTaskPriorities.forEach(p => renderItem(p, 'priority', priosList, customTaskPriorities, 'task_priorities'));
    ...
    document.getElementById('btnAddType').onclick = async () => {
        ...
        if (currentUser) await supabaseClient.from('task_types').upsert({ id: newItem.id, name: newItem.name, color: newItem.color, user_id: currentUser.id });
        lucide.createIcons();
    };
    ...
    document.getElementById('btnAddPrio').onclick = async () => {
        ...
        if (currentUser) await supabaseClient.from('task_priorities').upsert({ id: newItem.id, name: newItem.name, color: newItem.color, user_id: currentUser.id });
        lucide.createIcons();
    };
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (qm && qm.style.display === 'flex' && !qm.contains(e.target) && (!fab || !fab.contains(e.target))) {
